...
class BoardConfig:
    def __init__(self,
                ...
                min_track_to_track_distance = 0.3,
                plate_width = 200,
                plate_height = 200,
                padding_w = 10,
                padding_h = 10,
        ...
        self.min_track_to_track_distance = min_track_to_track_distance
...
def draw_pcb(config, file_name, lines, text=''):
    ...
    for i in range(61):
        ...
        if i % 10 == 0:
            line_hight = 10
        ruller_lines.append([(x_offset + i, y_offset ), (x_offset + i, y_offset + line_hight)])

    for line in ruller_lines:
        draw_line(dwg, line[0], line[1], 0.2, 'red')
...
def calc_width_for_power2(config, power, draw_method, tol = 1):
    cnt = 0
    while True:
        lines = draw_method(config.w,
                             config.h,
        ...
        r = calc_resistance(wire_lenth/1000, config.track_width, config.copper_thiknes_um, config.temperature)
        new_power = config.u*config.u/r
        config.track_to_track_distance += 0.1 if new_power < power else -0.1
        if config.track_to_track_distance <= config.min_track_to_track_distance:
            config.track_to_track_distance = config.min_track_to_track_distance
            config.track_width -= 0.1
        if abs(power-new_power)<=tol:
            return lines
        cnt += 1
        if cnt > 1000:
...
if __name__ == '__main__':
    ...
    for i, draw_function in enumerate(draw_functions):
        ...
        lines = calc_width_for_power2(config, 1800/4, draw_function)
